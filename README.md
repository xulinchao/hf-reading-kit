...
**口播 / 知识类视频** 的 HyperFrames（HF）跨项目素材库。  
读书稿是当前主力场景，但 kit 本身 **不锁死读书稿**。
...
| `blocks/` | **自研** | 验证过的 block（目前多来自读书稿项目） |
...
| `presets/` | **风格模板库** | 多 preset，见 [presets/README.md](presets/README.md) |
| `mapping.yaml` | 映射表 | 分镜 Skill → HF 实现（可选） |

---

## 色调 / 设计规范（frame.md）

HF 官方认 **项目根目录** 的 `frame.md`（设计系统），**不会**自动读 kit 里的文件。

**新项目定主色调：**

```powershell
copy presets\reading-watercolor\frame.md  ..\我的新项目\frame.md
# 编辑 frame.md 里的 colors.accent 等字段
```

| 文件 | 在哪 | 管什么 |
|------|------|--------|
| `kit/presets/*/frame.md` | 本仓库 | 风格 **模板** |
| `项目/frame.md` | 每个视频项目 | 本片 **锁定** 色板/字体 |
| `STORYBOARD.md` | 每个视频项目 | 分镜（见下文） |
...
| 自研 block | `blocks/<name>/` + 更新 `mapping.yaml` |
| 新风格 preset | `presets/<name>/frame.md`，见 presets/README.md |
| 灵感图 | 只放 `refs/` |
